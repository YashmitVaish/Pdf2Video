...
class RenderVideoInput(BaseModel):
    scenes : list[dict] = Field(...,description="Enter the list of the scenes that ae to be rendered")
    audio_path: str = Field(...,description="enter the path to the audio directory")
    video_path : str = Field(...,description="path to output dir")
...
class JSONScene(Scene):
    def __init__(self, scene_data: Dict, audio_dir: str, **kwargs):
...
def make_scene_class(scene_data, audio_dir):
    class_name = f"Scene_{scene_data.get('order', 0):03d}"
    def __init__(self, **kwargs):
        JSONScene.__init__(self, scene_data,audio_dir, **kwargs)
    return type(class_name, (JSONScene,), {"__init__": __init__})

from manim import config, tempconfig
from pathlib import Path

def render_scene_directly(scene_cls, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    output_path = Path(output_dir) / f"{scene_cls.__name__}.mp4"
...
class RenderVideoTool(BaseTool):
    ...
    description: str = "Renders JSON-driven scenes to video using Manim CLI. Returns scenes with video_path populated." 
    args_schema: Type[BaseModel] = RenderVideoInput

    def _run(self, scenes: list[Dict], audio_path: str = "test_audio_output", video_path: str = "video"):
        os.makedirs(video_path, exist_ok=True)
        for sc in scenes:
            cname = f"Scene_{sc.get('order', 0):03d}"
            globals()[cname] = make_scene_class(sc,audio_path)

        MANIM_FLAGS = ["-qh"]
        def render_all_directly():
            os.makedirs(video_path, exist_ok=True)
            for sc in scenes:
                cname = f"Scene_{sc.get('order', 0):03d}"
                cls = make_scene_class(sc, audio_path)
                render_scene_directly(cls, video_path)
